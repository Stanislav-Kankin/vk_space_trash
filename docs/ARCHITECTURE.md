...
Для закрытой альфы постоянный лом и улучшения временно сохраняются в browser `localStorage`; активная экспедиция не сохраняется. Это хранилище будет удалено из роли источника прогресса после появления серверной сессии. Production frontend собирается multi-stage Dockerfile и отдаётся Nginx-контейнером через loopback-порт хоста.
