...
Для закрытой альфы постоянный лом, улучшения и разведданные кораблей временно сохраняются в browser `localStorage`; активная экспедиция не сохраняется. Схема хранения версии 2 содержит для каждого `shipId` посещённые и зачищенные комнаты, а также флаг полного исследования. Состояние фиксируется только при успешной эвакуации. Это хранилище будет удалено из роли источника прогресса после появления серверной сессии. Production frontend собирается multi-stage Dockerfile и отдаётся Nginx-контейнером через loopback-порт хоста.
